...
#' @param X Covariate matrix (no intercept). Ignored if \code{outcome_type == "diff_in_diff"}.
#' @param identification String controlling the employed identification strategy. Must be one of \code{"selection_on_observables"}, \code{"diff_in_diff"}.
...
#'
#' @export
causalQual <- function(Y = NULL, Y_pre = NULL, Y_post = NULL, D = NULL, X = NULL, identification = NULL, outcome_type = NULL, K = 5) {
  ## 0.) Handling inputs and checks.
  if (any(!(D %in% c(0, 1)))) stop("Invalid 'D'. Treatment must be binary {0, 1}.", call. = FALSE)
  if (!(identification %in% c("selection_on_observables", "diff_in_diff"))) stop("Invalid 'identification'. Must be one of 'selection_on_observables', 'diff_in_diff'.", call. = FALSE)
  if (!(outcome_type %in% c("multinomial", "ordered"))) stop("Invalid 'outcome_type'. Must be either 'multinomial' or 'ordered'.", call. = FALSE)
  if (identification == "diff_in_diff" & (is.null(Y_pre) | is.null(Y_post))) stop("When 'identification' is 'diff_in_diff', we need both 'Y_pre' and 'Y_post'.", call. = FALSE)
  ...
  output$data <- list("Y" = Y, "Y_pre" = Y_pre, "Y_post" = Y_post, "D" = D, "X" = X)
