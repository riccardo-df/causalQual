...
#' @param identification String controlling the employed identification strategy. Must be one of \code{"selection_on_observables"}, \code{"iv"}, \code{"diff_in_diff"}, \code{"rd"}.
...
#'
#' @export
causalQual <- function(Y = NULL, Y_pre = NULL, Y_post = NULL, D = NULL, X = NULL, Z = NULL, running_variable = NULL, cutoff = NULL, identification = NULL, outcome_type = NULL, K = 5) {
  ## 0.) Handling inputs and checks.
  if (any(!(D %in% c(0, 1)))) stop("Invalid 'D'. Treatment must be binary {0, 1}.", call. = FALSE)
  if (!(identification %in% c("selection_on_observables", "iv", "diff_in_diff", "rd"))) stop("Invalid 'identification'. Must be one of 'selection_on_observables', 'iv', 'diff_in_diff', 'rd'.", call. = FALSE)
  if (identification != "iv" & (!is.null(Z))) stop("When 'identification' is not 'iv', we do not need 'Z'.", call. = FALSE)
  ...
  if (identification != "diff_in_diff" & (!is.null(Y_pre) | !is.null(Y_post))) stop("When 'identification' is not 'diff_in_diff', we do not need 'Y_pre' and 'Y_post'. Rather, please provide 'Y'.", call. = FALSE)
  ...
  if (identification == "rd" & (!is.null(D))) stop("When 'identification' is 'rd', we do not need 'D'.", call. = FALSE)
  if (identification == "rd" & (!is.null(X))) stop("When 'identification' is 'rd', we do not need 'X'.", call. = FALSE)
  if (identification != "rd" & (!is.null(cutoff))) stop("When 'identification' is not 'rd', we do not need 'cutoff'.", call. = FALSE)
  ...
  output$confidence_intervals <- fit$confidence_intervals
